...
export async function GET() {
  ...
  try {
    // 울산 남구(31140) 주차장 기본정보 및 실시간 현황 수신
    const ulsanCode = '31140';
    const infoUrl = `${PARKING_INFO_API_URL}?addr_cd=${ulsanCode}&addr_type=SIGUNGU&pageNo=1&pageSize=1000`;
    const statusUrl = `${PARKING_STATUS_API_URL}?addr_cd=${ulsanCode}&addr_type=SIGUNGU&pageNo=1&pageSize=1000`;
    ...
    const ulsanParkings = baseArr.slice(0, 15).map((info: any) => {
    ...
    return NextResponse.json({
      success: true,
      description: '울산 남구(31140) 주차장 실시간 수신 현황 진단 결과',
      ulsanParkings,
    });
  } catch (err: any) {
