...
import { Users, Car, MapPin, Calendar, ChevronRight } from 'lucide-react';
...
export default function FestivalCarousel({
  ...
  if (festivals.length === 0) {
    return (
      <div className="w-full py-4 px-4 bg-white/90 backdrop-blur-md rounded-2xl shadow-lg border border-slate-200 text-center text-xs text-slate-500">
        해당 권역에 등록된 축제가 없습니다.
      </div>
    );
  }
    ...
    <div className="w-full">
      ...
      >
        {festivals.map((fest) => {
          ...
          // 가장 가까운 주차장 찾기
          const nearestParking = fest.parkingLots.reduce((prev, curr) =>
          ...
          return (
            <div
              ...
              className={clsx(
                'snap-center shrink-0 w-[290px] p-3.5 bg-white/95 backdrop-blur-md rounded-2xl border transition-all duration-200 shadow-md cursor-pointer flex flex-col justify-between',
                isSelected
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <div>
                    ...
                    <div className="text-xs font-bold text-indigo-900 mt-0.5">
                      잔여 <span className="text-indigo-600 font-extrabold">{nearestParking.availableSpaces}</span> / {nearestParking.totalSpaces}면
                    </div>
                  </div>
                </div>
